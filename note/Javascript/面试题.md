...
watch: 监听属性，监听的目标发生变化后，在单独的回调函数中执行副作用，值改变后才会调用
...
```js
1. 面试官你好，我叫xxx,目前就读于西安工程大学计算机专业，读研期间层获得过1项省奖，2项校奖。参与过导师的两个项目，一个是高校智能化教学评估，通过深度学习技术对实时课堂教学视频进行表情与姿态识别，通过评价指标对识别结果进行分析，起到辅助教学评估的作用。第二个是对澳洲发电量和电价进行预测，提前感知shi'chang'bi
```
