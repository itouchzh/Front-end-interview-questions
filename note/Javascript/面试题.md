...
> 5. 重排和重绘：当浏览器需要更新页面内容时，它会重新执行上述步骤，因为布局和绘制的过程是比较耗时的，所以浏览器会尽可能的避免重排和重绘，通过一些优化技巧，如合并重绘区域、使用 GPU 加速等来提高页面的渲染性能。ang1
...
http1.0使用短连接、每次请求都要重新建立连接。
http1.1 引入了持久连接，默认不关闭连接，可以被多个请求复用。但是需要排队发送，可能会发生拥塞问题
http2.0 实现了完全多路复用，引入了头部压缩机制，允许未经请求，向客户端发送资源。
```
...
```js
我的缺点是工作需要压力， 在有要求、 有竞争的时候我效率更高。我的学习成绩很好， 因为有考试的压力。但是在大学里面， 我没有做太多的兼职活动， 因为没有赚钱的压力。所以我特别希望自己的老板能比较严格， 多给我一些压力，。
```
